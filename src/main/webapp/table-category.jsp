<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html lang="vi">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>ECom Website</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/css/bootstrap.min.css" rel="stylesheet">
    <link href="./style.css" rel="stylesheet">
</head>

<body class="d-flex flex-column min-vh-100">

    <!-- Header -->
    <jsp:include page="./partial/header.jsp"></jsp:include>

    <!-- Nội dung chính -->
    <main class="flex-grow-1">
        <div class="container mt-5 my-5">
            <h1 class="mb-4 text-center">Loại sản phẩm</h1>

            <div class="items-table table-responsive">
                <table class="table table-bordered align-middle text-center">
                    <thead class="table-light table-sticky-header">
                        <tr>
                            <th>Mã loại sản phẩm</th>
                            <th>Tên loại sản phẩm</th>
                            <th>Thao tác</th>
                        </tr>
                    </thead>
                    <tbody>
                        <!-- Render category -->
                        <tr>
                            <td>1</td>
                            <td>Loại Số 1</td>
                            <td><button class="btn btn-primary">Xem</button></td>
                        </tr>
                        <tr>
                            <td>1</td>
                            <td>Loại Số 1</td>
                            <td><button class="btn btn-primary">Xem</button></td>
                        </tr>
                        <tr>
                            <td>1</td>
                            <td>Loại Số 1</td>
                            <td><button class="btn btn-primary">Xem</button></td>
                        </tr>
                        <tr>
                            <td>1</td>
                            <td>Loại Số 1</td>
                            <td><button class="btn btn-primary">Xem</button></td>
                        </tr>
                        <tr>
                            <td>1</td>
                            <td>Loại Số 1</td>
                            <td><button class="btn btn-primary">Xem</button></td>
                        </tr>
                        <tr>
                            <td>1</td>
                            <td>Loại Số 1</td>
                            <td><button class="btn btn-primary">Xem</button></td>
                        </tr>
                        <tr>
                            <td>1</td>
                            <td>Loại Số 1</td>
                            <td><button class="btn btn-primary">Xem</button></td>
                        </tr>
                        <tr>
                            <td>1</td>
                            <td>Loại Số 1</td>
                            <td><button class="btn btn-primary">Xem</button></td>
                        </tr>
                        <tr>
                            <td>1</td>
                            <td>Loại Số 1</td>
                            <td><button class="btn btn-primary">Xem</button></td>
                        </tr>
                        <tr>
                            <td>1</td>
                            <td>Loại Số 1</td>
                            <td><button class="btn btn-primary">Xem</button></td>
                        </tr>
                        <tr>
                            <td>1</td>
                            <td>Loại Số 1</td>
                            <td><button class="btn btn-primary">Xem</button></td>
                        </tr>
                        <tr>
                            <td>1</td>
                            <td>Loại Số 1</td>
                            <td><button class="btn btn-primary">Xem</button></td>
                        </tr>
                        <tr>
                            <td>1</td>
                            <td>Loại Số 1</td>
                            <td><button class="btn btn-primary">Xem</button></td>
                        </tr>
                        <tr>
                            <td>1</td>
                            <td>Loại Số 1</td>
                            <td><button class="btn btn-primary">Xem</button></td>
                        </tr>
                        <tr>
                            <td>1</td>
                            <td>Loại Số 1</td>
                            <td><button class="btn btn-primary">Xem</button></td>
                        </tr>
                    </tbody>
                </table>
            </div>

            <hr>
            
            <form>
                <div class="row mt-5">
                    <div class="col-md-6">
                        <div class="row mb-3">
                            <label for="inputCategoryId" class="form-label">Mã loại sản phẩm</label>
                            <div class="row">
                                <div class="col-md-12">
                                    <input type="text" class="form-control" id="inputCategoryId"
                                        placeholder="Nhập mã loại sản phẩm">
                                </div>
                            </div>
                        </div>
                        <div class="row mb-3">
                            <label for="inputName" class="form-label">Tên loại sản phẩm</label>
                            <div class="row">
                                <div class="col-md-12">
                                    <input type="text" class="form-control" id="inputName"
                                        placeholder="Nhập tên loại sản phẩm">
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </form>

            <div class="row mt-5">
                <div class="col-md-3 mb-3">
                    <button type="button" class="btn btn-secondary w-100">Làm mới</button>
                </div>
                <div class="col-md-3 mb-3">
                    <button type="button" class="btn btn-primary w-100">Tạo</button>
                </div>
                <div class="col-md-3 mb-3">
                    <button type="button" class="btn btn-success w-100">Cập nhật</button>
                </div>
                <div class="col-md-3 mb-3">
                    <button type="button" class="btn btn-danger w-100">Xoá</button>
                </div>
            </div>
        </div>
    </main>

    <!-- Footer -->
    <jsp:include page="./partial/footer.jsp"></jsp:include>

    <!-- Bootstrap JS -->
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/js/bootstrap.bundle.min.js"></script>
</body>

</html>